...
export function getCustomList_OLD(type: 'hiragana' | 'katakana'): KanaCharacter[] {
...
export function saveCustomList_OLD(type: 'hiragana' | 'katakana', cards: KanaCharacter[]): void {
...
export function resetToDefault(type: 'hiragana' | 'katakana'): KanaCharacter[] {
  ...
  const lists = getAllCustomLists();
  const migratedList = lists.find(l => l.type === type && l.name.includes('(migrert)'));
